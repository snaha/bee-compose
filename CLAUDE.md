...
**The Anvil snapshot is the single source of truth for chain state at runtime.** `blockchain/state.gnosis.json` is plain JSON (~800 KB): the BZZ token, WXDAI and the SushiSwap router / quoter / BZZ pool borrowed from a Gnosis mainnet fork, plus PostageStamp / PriceOracle / StakeRegistry / Redistribution **deployed from source** onto their mainnet addresses, and the 9 Bee node EOAs pre-funded with 100 xDAI for gas and 10 BZZ for their own postage.
...
**The dev faucet is where value comes from, not the nodes.** `0xF406AebbF610A9c54589e7EbE25b8e6621258410`, key `keccak256("bee-compose dev faucet")`, baked holding 100 xDAI and 250 BZZ. Fund test addresses by transferring from it rather than swapping — the BZZ/WXDAI pool is real and genuinely thin (~180 WXDAI against 19 300 BZZ, ~$1.2k), and it is shared by every purchase this chain will ever serve; ~0.5 xDAI of buying moves the price ~0.6%. The Bee node EOAs get gas plus a small BZZ float for their own stamps and nothing more; don't park test value on a node's wallet.
...
deploy transactions itself. Nothing under `blockchain/deploy/` or
`blockchain/bake/` ships in the published tarball — re-baking needs a git
checkout and internet, and consumers get the committed snapshot.
...
- **`BEE_POSTAGE_STAMP_START_BLOCK` moves with every bake.** It is the block PostageStamp was deployed in. Set it *later* and the nodes miss the initial `PriceUpdate` and every `BatchCreated` — batches exist on chain and no node has heard of them, with no error anywhere. `blockchain/bake/finalise.ts` writes it into `compose.yml` for exactly that reason; commit the snapshot and that edit together, and don't hand-edit it back.
- **A borrowed contract's read-only storage has to be pinned, or it reads as zero.** A forked anvil fetches state lazily and dumps only what something *wrote* to, so `BZZ.decimals()` came back `0` and `symbol()` empty on an early snapshot — silently, since a missing slot is zero rather than an error, and every consumer then formats BZZ amounts 1e16 out. `warmReads()` in `blockchain/bake/chain.ts` traces each such call with `prestateTracer` and writes every slot it touches back at its own value, which marks it dirty so it lands in the dump; `deploy-swarm.ts` asserts the result after the reload. Anything else the chain must *answer* rather than *execute* needs the same treatment — a whole missing contract (the Sushi quoter) is the `READ_ONLY_CONTRACTS` splice in `finalise.ts` instead.
